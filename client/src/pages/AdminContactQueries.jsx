...
import { getContactQueries, replyToContactQuery, closeContactQuery, markQueryRead } from "../services/contactService";
...
const AdminContactQueries = () => {
  ...
  const handleSelectQuery = async (q) => {
    ...
    if (!q.isRead) {
      try {
        const updated = await markQueryRead(q.id);
        setQueries((prev) => prev.map((item) => (item.id === updated.id ? updated : item)));
        setSelected(updated);
        // Same pattern chat/notifications use to make the sidebar badge
        // update instantly instead of waiting for its next 20s poll.
        window.dispatchEvent(new Event("crm_queries_updated"));
      } catch (err) {
          ...
          >
            <div className="d-flex justify-content-between align-items-start">
              <div className="fw-medium d-flex align-items-center gap-2">
                {!q.isRead && (
                  <span
                    style={{ width: "8px", height: "8px", borderRadius: "50%", background: "#dc3545", flexShrink: 0 }}
                    title="Unread"
                  />
                )}
                {q.name}
