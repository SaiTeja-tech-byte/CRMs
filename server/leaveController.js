...
// Fixed annual entitlement — there's no per-employee leave policy table yet,
// so every employee is treated the same way. Easy to swap for a real
// CompanySettings-driven value later without touching the routes/controllers.
...
const createLeaveRequest = async (req, res) => {
  try {
    ...
    // Notify every admin — same fan-out pattern as assignTask/assignEvent,
    // just in the other direction (employee -> admins).
...
// GET /api/leave/mine — the logged-in employee's own requests.
...
// GET /api/leave/balance — the logged-in employee's remaining leave this
// calendar year (entitlement minus days already Approved).
...
// GET /api/leave — admin: every employee's leave requests (optionally
...
// PATCH /api/leave/:id — admin: approve or reject a pending request.
